...
def get_good_units(analyzer):
    ...
    good_qms = qms.query("snr > 1 & firing_rate > 0.05 & rp_contamination < 0.2")
    return good_qms


def get_type_of_firing(spike, rec_samples, threshold=0.1):
 
    num_of1spikes = np.sum(spike < rec_samples['of1'])/rec_samples['of1']*30_000
    num_vrspikes = np.sum( (rec_samples['of1']+rec_samples['vr'] > spike) & (spike > rec_samples['of1']) )/rec_samples['vr']*30_000
    num_of2spikes = np.sum(spike > rec_samples['of1']+rec_samples['vr'])/rec_samples['of2']*30_000
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, sorting_analyzer):
        ...
        self.template_similarity = sorting_analyzer.get_extension("template_similarity").get_data()
        ...
        self.templates = {unit_id_1: 
            templates_data[unit_id_1,:,max_channels[sorting_analyzer.sorting.id_to_index(unit_id_1)]]
            for unit_id_1 in sorting_analyzer.unit_ids }
        self.all_templates = {unit_id_1: 
            templates_data[unit_id_1,:,self.sparsity_mask[sorting_analyzer.sorting.id_to_index(unit_id_1)]]
            for unit_id_1 in sorting_analyzer.unit_ids }
        ...
        possible_units = get_similar_units(self.template_similarity, self.unit_ids, self.unit_id_1) 
    ...
    def initialise_plot(self):
        ...
        self.unit_locations_plot_3 = self.unit_locations_widget.plot(symbol="x", symbolSize=20, symbolBrush=unit_1_color)
        self.unit_locations_plot_4 = self.unit_locations_widget.plot(symbol="x", symbolSize=20, symbolBrush=unit_2_color)

        self.templates_1_plot = self.template_widget.plot(pen=pg.mkPen( unit_1_color, width=3 ))
        self.templates_2_plot = self.template_widget.plot(pen=pg.mkPen( unit_2_color, width=3 ))
        
        self.amps_plot_1 = self.amps_1_widget.plot(pen=None, symbolPen =None, symbol="o", symbolBrush = unit_1_color, symbolSize=4)
        self.amps_plot_2 = self.amps_2_widget.plot(pen=None, symbolPen =None, symbol="o", symbolBrush = unit_2_color, symbolSize=4)

        self.correlogram_11_plot = self.correlogram_11_widget.plot(stepMode="left", fillLevel=0, fillOutline=True, brush=(0,0,255,150))
        self.correlogram_12_plot = self.correlogram_12_widget.plot(stepMode="left", fillLevel=0, fillOutline=True, brush=(0,0,255,150))
        self.correlogram_21_plot = self.correlogram_21_widget.plot(stepMode="left", fillLevel=0, fillOutline=True, brush=(0,0,255,150))
        self.correlogram_22_plot = self.correlogram_22_widget.plot(stepMode="left", fillLevel=0, fillOutline=True, brush=(0,0,255,150))

        self.all_templates_1_plot = self.all_templates_widget.plot(pen=pg.mkPen( unit_1_color, width=2 ))
        self.all_templates_2_plot = self.all_templates_widget.plot(pen=pg.mkPen( unit_2_color, width=2 ))
    ...
    def update_plot(self, unit_data):
        ...
        self.correlogram_11_plot.setData(np.arange(0,len(unit_data['correlogram_11']),1), unit_data['correlogram_11'])
        self.correlogram_12_plot.setData(np.arange(0,len(unit_data['correlogram_12']),1), unit_data['correlogram_12'])
        self.correlogram_21_plot.setData(np.arange(0,len(unit_data['correlogram_21']),1), unit_data['correlogram_21'])
        self.correlogram_22_plot.setData(np.arange(0,len(unit_data['correlogram_22']),1), unit_data['correlogram_22'])

        self.unit_locations_plot_3.setData([unit_data['unit_location_1'][0]],[unit_data['unit_location_1'][1]])
        self.unit_locations_plot_4.setData([unit_data['unit_location_2'][0]],[unit_data['unit_location_2'][1]])

        self.all_templates_widget.clear()
        self.update_template_plot(channel_locations, unit_data['all_template_1'], unit_data['all_template_2'])
    ...
    def update_template_plot(self, channel_locations, all_template_1, all_template_2):
        ...
        for template_index, template_channel_loc in enumerate(template_channels_locs_1):            
            curve = pg.PlotCurveItem(4*template_channel_loc[0] + np.arange(90), template_channel_loc[1]/1 + all_template_1[template_index,:],pen=pg.mkPen(unit_2_color, width=2))
            self.all_templates_widget.addItem(curve)

        for template_index, template_channel_loc in enumerate(template_channels_locs_2):            
            curve = pg.PlotCurveItem(4*template_channel_loc[0] + np.arange(90), template_channel_loc[1]/1 + all_template_2[template_index,:],pen=pg.mkPen(unit_1_color, width=2))
            self.all_templates_widget.addItem(curve)
